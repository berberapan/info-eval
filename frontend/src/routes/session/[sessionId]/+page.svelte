<script>
  ...
  import { onMount } from 'svelte'; // 'browser' import removed as it was unused and from incorrect source
  ...
  async function fetchScenarioData(sessionId) {
    ...
    try {
      const response = await fetch(`${BASE_API_URL}${sessionId}`);

      if (!response.ok) {
        let errorResponseMessage = response.statusText; // Default message
        try {
          // Try to get a more specific message if the error response is JSON
          const errorData = await response.json();
          errorResponseMessage = errorData.message || errorData.detail || errorData.error || response.statusText;
        } catch (jsonError) {
          // If response is not JSON or .json() fails, try to read as text for a simpler error
          try {
            const textError = await response.text();
            // Only use textError if it's relatively short, to avoid displaying large HTML error pages
        ...
        throw new Error(`API-fel! Status: ${response.status} - ${errorResponseMessage}`);
      }
      ...
      try {
        ...
        throw new Error("Kunde inte tolka svar från API. Förväntade JSON.");
      }


      if (data && data.scenario) { // Assuming your API nests the scenario object
        ...
        // Initialize answers for the first exercise if data is loaded
        initializeAnswersForCurrentExercise();
      } else if (data && !data.scenario) { // If API returns scenario directly at root
        ...
        scenarioData = data; // Attempt to use data directly as scenario object
         if (scenarioData.exercises && scenarioData.exercises.length > 0) {
          scenarioData.exercises.sort((a, b) => a.order - b.order);
        }
        initializeAnswersForCurrentExercise();
      ...
      console.error("Misslyckades med att hämta eller bearbeta scenariodata:", e);
      error = e.message || "Ett okänt fel uppstod vid hämtning av scenario.";
      // Ensure scenarioData is cleared on error to prevent rendering stale or partial data
      scenarioData = null;
    } finally {
      isLoading = false;
    }
  ...
  function initializeAnswersForCurrentExercise() {
    ...
    selectedMediaTab = 0; // Reset media tab to first
  }
  ...
  function submitAnswers() {
    ...
    console.log("Skickar svar för övning:", currentExercise.id, answers);
    // Placeholder: In a real app, send 'answers' to your backend for validation.
    // For now, we'll just show feedback locally.
    ...
    currentExercise.questions.forEach(q => {
      if (q.type === 'true_false' || q.type === 'multiple_choice') {
        ...
        if (answers[q.id] && answers[q.id].trim() !== '') {
            q.submittedFeedback = "Svar för fritext mottaget. (Verklig bedömning skulle ske på servern)";
    ...
    {#if currentExercise}
      <div class="card bg-base-100 shadow-xl">
        <div class="card-body">
          ...
          {/if}
          {#if currentExercise.exercise_description} <p class="mb-4 text-base-content/90">{currentExercise.exercise_description}</p>
          {/if}

          <div class="flex flex-col md:flex-row gap-6 mt-2">
            <div class="w-full {currentMedia && currentMedia.length > 0 ? 'md:w-1/2' : 'md:w-full'}">
            ...
            {#if currentMedia && currentMedia.length > 0}
              <div class="w-full md:w-1/2 md:sticky md:top-8"> {#if currentMedia.length > 1}
                  <div role="tablist" class="tabs tabs-lifted tabs-lg mb-2">
                    {#each currentMedia as mediaItem, i (mediaItem.id)}
                      <button
                        role="tab"
                        aria-label="Media {i+1}"
                        class="tab {selectedMediaTab === i ? 'tab-active font-semibold !bg-base-100' : 'bg-base-200/50'}"
                        on:click={() => selectedMediaTab = i}>
                        Media {i + 1}
                      </button>
                    {/each}
                  </div>
                {/if}
                {#each currentMedia as mediaItem, i (mediaItem.id)}
                  <div class="p-1 bg-base-100 rounded-b-lg {selectedMediaTab === i ? 'block' : 'hidden'}">
                    {#if mediaItem.media_type === 'image'}
                      <img
                        src={mediaItem.media_url}
                        alt="Scenario media {i+1}: {mediaItem.caption || 'Bild'}"
                    ...
                    {:else if mediaItem.media_type === 'video'}
                       <video controls class="rounded-lg shadow-md w-full max-h-[70vh]">
                           <source src={mediaItem.media_url} type="video/mp4" /> <track kind="captions" />
                    ...
                    {:else}
                      <div class="p-4 bg-warning/20 rounded-lg text-warning-content">
                        <p class="font-semibold">Okänd mediatyp: {mediaItem.media_type}</p>
                        <p class="text-sm">URL: {mediaItem.media_url}</p>
                      </div>
                    {/if}
                  </div>
                {/each}
...
<style>
  ...
  /* Ensure sticky positioning works as expected if you use it */
  .md\:sticky {
